...
bool ImGui::CustomButton(const char* label, const char* text, float sizeX, float sizeY) {
    ...
    ImVec4 target = ImGui::IsItemActive() ? ImColor(60, 120, 255) : ImGui::IsItemHovered() ? ImColor(60, 120, 255) : ImColor(60, 120, 255);
    ...
    ImVec4 targetText = ImGui::IsItemActive() || ImGui::IsItemHovered() ? ImColor(255, 255, 255) : ImColor(200, 200, 200);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    const ImU32 grab_col = ImColor(60, 120, 255);
...
bool ImGui::CustomCheckbox(const char* label, ImColor color, bool* v)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    window->DrawList->AddRectFilled(total_bb.Min, total_bb.Min + ImVec2(square_sz, square_sz), ImColor(60 / 255.0f, 120 / 255.0f, 255 / 255.0f, it_anim->second.selected_rect), 3.0f);
    window->DrawList->AddShadowRect(total_bb.Min, total_bb.Min + ImVec2(square_sz, square_sz), ImColor(60 / 255.0f, 120 / 255.0f, 255 / 255.0f, it_anim->second.selected_rect), 20, ImVec2(0, 0), 0, 3.f);

    RenderCheckMark(window->DrawList, ImVec2(total_bb.Min.x + 5, (total_bb.Min.y + total_bb.Max.y) / 2 - 5), ImColor(1.0f, 1.0f, 1.0f, it_anim->second.check_rect), 10.0f);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    // Manage slider animation using map
    static std::map<ImGuiID, slider_element> anim;
    auto [it_anim, inserted] = anim.try_emplace(id, slider_element{ 0.0f });
    ...
    static float SliderAnim = 10;
    SliderAnim = ImLerp(SliderAnim, grab_bb.Min.x - frame_bb.Min.x, ImGui::GetIO().DeltaTime * 12);

    // Draw slider background and value
    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(15, 15, 16), 5.0f);
     window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(grab_bb.Min.x + 6, frame_bb.Max.y), ImColor(60, 120, 255), 5.0f);
    window->DrawList->AddShadowRect(frame_bb.Min, ImVec2(grab_bb.Min.x + 6, frame_bb.Max.y), ImColor(60, 120, 255), 20, ImVec2(0, 0), 0, 3.f);
    window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x + 3, frame_bb.Max.y - 3), 5.0f, ImColor(60, 120, 255), 100);
...
bool ImGui::CustomInputText(const char* label, const char* hint, float posX, float posY, float sizeX, static char buf[], size_t buf_size, ImGuiInputTextFlags flag)
{
    ...
    window->DrawList->AddShadowRect({ pos.x + MIN.x, pos.y + MAX.y - 1 }, MAX + pos, ImGui::GetColorU32(it_anim->second.ShadowColor), it_anim->second.Shadow_size, { 0,0 }, 0, 2.0f);
    window->DrawList->AddLine({ pos.x + MIN.x, pos.y + MAX.y }, {pos.x + MAX.x, pos.y + MAX.y }, ImColor(60, 120, 255), 1.f);
...
bool ImGui::CustomTab(const char* name, float posX, float posY, bool boolean)
{
    ...
    const ImRect rect(pos, ImVec2(pos.x + 40, pos.y + 40));
    ...
    // it_anim->second.textColor = ImLerp(it_anim->second.textColor, boolean ? ImColor(144, 151, 141) : hovered ? ImColor(144, 151, 141, 200) : ImColor(255, 255, 255), ImGui::GetIO().DeltaTime * 6.f);

    window->DrawList->AddRectFilled(rect.Min, rect.Max, ImColor(1.0f, 1.0f, 1.0f, it_anim->second.element_opacity), 3.0f);

    window->DrawList->AddText(ImVec2(iconpos.Min.x + posX, (iconpos.Min.y + iconpos.Max.y) / 2 - label_size.y / 2), ImColor(1.0f, 1.0f, 1.0f, it_anim->second.text_opacity), name);

    window->DrawList->AddRectFilled(ImVec2(rect.Max.x + 5, rect.Min.y + 9), ImVec2(rect.Max.x + 8, rect.Max.y - 9), ImColor(60 / 255.0f, 120 / 255.0f, 255 / 255.0f, it_anim->second.rect_opacity), 10.0f, ImDrawFlags_RoundCornersLeft);
    window->DrawList->AddShadowRect(ImVec2(rect.Max.x + 5, rect.Min.y + 9), ImVec2(rect.Max.x + 8, rect.Max.y - 9), ImColor(60 / 255.0f, 120 / 255.0f, 255 / 255.0f, it_anim->second.rect_opacity), 10, ImVec2(0, 0), 0, 3.f);
